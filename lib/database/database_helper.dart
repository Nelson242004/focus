...
class DatabaseHelper {
    ...
    return openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        if (oldVersion < 4) {
          ...
          ''');
          await _safeAlter(
              db, 'ALTER TABLE subjects ADD COLUMN defaultClassroom TEXT');
        }
        if (oldVersion < 5) {
          await _safeAlter(
        ...
        if (oldVersion < 9) {
          await _safeAlter(
              db, 'ALTER TABLE resources ADD COLUMN subjectId INTEGER');
        }
        if (oldVersion < 10) {
          await _safeAlter(
            db,
        ...
        if (oldVersion < 12) {
          await _safeAlter(
              db, 'ALTER TABLE subjects ADD COLUMN professorName TEXT');
          await _safeAlter(
              db, 'ALTER TABLE subjects ADD COLUMN sectionCode TEXT');
        }
        if (oldVersion < 13) {
  ...
  Future<void> _insertDefaultData(Database db) async {
    ...
    await db.insert(
        'subjects', {'name': 'Física', 'color': '#f59e0b', 'icon': 'science'});
    await db.insert(
      'settings',
  ...
  Future<int> deleteSubject(int id) async {
    ...
    await db.rawUpdate(
        'UPDATE exams SET subjectId = NULL WHERE subjectId = ?', [id]);
  ...
  Future<void> updateSettings(AppSettings settings) async {
    final db = await database;
    await db.update('settings', {'value': _serializeSettings(settings)},
        where: 'key = ?', whereArgs: ['settings']);
  }
