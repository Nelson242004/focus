...
class NotificationService {
  ...
  static Future<void> showPomodoroTimerNotification({
    ...
    final title = isFocus
        ? 'Focus • Enfoque'
        : isLongBreak
            ? 'Focus • Descanso largo'
            : 'Focus • Descanso corto';
    final body = isFocus
        ? (subject.trim().isEmpty
            ? 'Sesión de estudio en curso'
            : 'Estudiando $subject')
        : 'Respira. El siguiente bloque empezará automáticamente.';
    final endAt = DateTime.now().add(Duration(seconds: remainingSeconds));
